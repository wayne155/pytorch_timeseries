"""Tests for FastFormerForecaster — Fastformer additive attention (O(T·d) complexity)."""
import pytest
import torch

from torch_timeseries.model.FastFormerForecaster import (
...
class TestFastAttentionBlock:
    def test_output_shape(self):
        block = _FastAttentionBlock(d_model=D, n_heads=H, dropout=0.0)
    ...
    def test_invalid_heads(self):
        with pytest.raises(AssertionError):
            _FastAttentionBlock(d_model=D, n_heads=3, dropout=0.0)  # 16 % 3 != 0
...
class TestFastFormerLayer:
    def test_output_shape(self):
        layer = _FastFormerLayer(d_model=D, n_heads=H, d_ffn=64, dropout=0.0)
...
class TestFastFormerForecaster:
    def _model(self, **kw):
        defaults = dict(
        ...
        defaults.update(kw)
        return FastFormerForecaster(**defaults)
    ...
    def test_importable_from_package(self):
        from torch_timeseries.model import FastFormerForecaster as FF
        assert FF is FastFormerForecaster
    ...
    def test_longer_seq(self):
        m = FastFormerForecaster(
            seq_len=96, pred_len=PRED, enc_in=C,
            d_model=D, n_heads=H, d_ffn=64
        )
        assert m(torch.randn(B, 96, C)).shape == (B, PRED, C)

    def test_additive_attention_linear_in_T(self):
        """FastFormer does NOT allocate a (B,T,T) attention matrix — param count independent of seq_len."""
        m16 = FastFormerForecaster(seq_len=16, pred_len=PRED, enc_in=C, d_model=D, n_heads=H, d_ffn=64)
        m64 = FastFormerForecaster(seq_len=64, pred_len=PRED, enc_in=C, d_model=D, n_heads=H, d_ffn=64)
        n16 = sum(p.numel() for p in m16.parameters())
        n64 = sum(p.numel() for p in m64.parameters())
        assert n16 == n64  # param count independent of seq_len
