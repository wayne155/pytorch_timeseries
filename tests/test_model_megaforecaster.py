"""Tests for MEGAForecaster — Moving-Average Equipped Gated Attention."""
import pytest
import torch

from torch_timeseries.model.MEGAForecaster import (
    MEGAForecaster, _EMALayer, _GSAHBlock,
)
...
class TestEMALayer:
    def test_output_shape(self):
        ema = _EMALayer(d_model=D)
        assert ema(torch.randn(B, SEQ, D)).shape == (B, SEQ, D)

    def test_output_finite(self):
        ema = _EMALayer(d_model=D)
        assert torch.isfinite(ema(torch.randn(B, SEQ, D))).all()

    def test_beta_in_0_1(self):
        ema = _EMALayer(d_model=D)
        beta = torch.sigmoid(ema.beta_raw)
    ...
    def test_longer_seq(self):
        ema = _EMALayer(d_model=D)
        assert ema(torch.randn(B, 96, D)).shape == (B, 96, D)


class TestGSAHBlock:
    def test_output_shape(self):
        block = _GSAHBlock(d_model=D, d_expand=8, dropout=0.0)
...
class TestMEGAForecaster:
    def _model(self, **kw):
        defaults = dict(seq_len=SEQ, pred_len=PRED, enc_in=C,
                        d_model=D, d_expand=8, n_layers=2, dropout=0.0)
        defaults.update(kw)
        return MEGAForecaster(**defaults)
    ...
    def test_ema_beta_has_grad(self):
        m = self._model()
        m(torch.randn(B, SEQ, C)).sum().backward()
        for i, block in enumerate(m.blocks):
            assert block.ema.beta_raw.grad is not None, f"No grad for blocks[{i}].ema.beta_raw"
    ...
    def test_importable_from_package(self):
        from torch_timeseries.model import MEGAForecaster as MEGA
        assert MEGA is MEGAForecaster
    ...
    def test_larger_d_model(self):
        m = self._model(d_model=64, d_expand=32)
        assert m(torch.randn(B, SEQ, C)).shape == (B, PRED, C)

    def test_longer_seq(self):
        m = MEGAForecaster(seq_len=48, pred_len=PRED, enc_in=C, d_model=D, d_expand=8)
