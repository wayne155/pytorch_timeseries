...
/** Explains the metric sub-columns for each view, shown at the table's top-left. */
const COLUMNS_NOTE: Record<string, string> = {
  long_term_forecast:
    'Sub-columns 96 / 192 / 336 / 720 = prediction length (pred_len); avg = mean over all prediction lengths. Input window is set by the variant above.',
  short_term_forecast:
    'Sub-columns 24 / 48 / 96 = prediction length (pred_len); avg = mean over all prediction lengths.',
  imputation:
    'Sub-columns 0.125 / 0.25 / 0.375 / 0.5 = mask rate (fraction of values hidden); avg = mean over all mask rates.',
}
  ...
  return (
    <div className="min-h-screen flex flex-col font-mono">
      ...
      <header className="relative overflow-hidden border-b border-line bg-surface-panel">
        ...
        <div className="relative flex items-end justify-between px-6 pt-5 pb-4">
          <div className="flex items-end gap-3.5">
            <img src={`${import.meta.env.BASE_URL}favicon.svg`} alt="torch-timeseries logo" className="h-12 w-12" />
            <div>
              ...
              <div className="flex flex-1 min-h-0">
                ...
                <main className="flex-1 overflow-auto p-5">
                  <TaskTable
                    ...
                    columnsNote={COLUMNS_NOTE[selectedView.id]}
                  />
                </main>
              </div>
            </>
      ...
      <footer className="border-t border-line bg-surface-panel px-6 py-2 text-xs text-ink-dim flex justify-between">
        <span>
          ▲ best per column · bar = % of column best (100% = best) · lower&nbsp;↓ or higher&nbsp;↑ is better
        </span>
        <a
